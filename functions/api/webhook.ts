...
// (UI buton action-hook'ları AYRI uçta: /api/action — senkron JSON. Webhook yalnız async event'ler içindir.)
//
// Akış: ham gövde → serverId → install.webhookSecret ile HMAC doğrula → idempotency →
//       mapEventPayload → courierUrl'e POST → başarı 200 (+markProcessed) / başarısız 5xx (retry, mark YOK).
